...
<div class="container">
    ...
    <p>Here the given {equation == 'ODE' ? "Ordinary Diffrential Equation" : "Partial Diffrential Equation"} is to be solved by {data.otherInfo.name} with inital x,y i.e <strong>x<sub>0</sub> = {lowerLimit} and y<sub>0</sub> = {initialValue}</strong> and
        the number of successive steps N = { Math.round((upperLimit - lowerLimit) / h) }.</p>
